...
export class Platform extends HomebridgePlatform<Configuration> {
  ...
  public initialize() {
    ...
    for (const groupConfiguration of this.configuration.groups) {
      if (groupConfiguration.name && groupConfiguration.switches) {
        ...
        if (groupConfiguration.switches.some((s) => !s.name)) {
          this.logger.warn(
            `[${groupConfiguration.name}] Switches are not configured properly.`,
          );
          continue;
        }
        if (
          groupConfiguration.switches.filter((s) => s.isDefaultOn).length > 1
        ) {
          this.logger.warn(
            `[${groupConfiguration.name}] Multiple switches are set as default on. This is not a valid configuration.`,
          );
          continue;
        }
