...
export class GroupController {
  ...
  constructor(
    ...
    const accessory = platform.useAccessory(
      groupConfiguration.name,
      `${groupConfiguration.name}-group`,
    );
    ...
    for (const switchConfiguration of groupConfiguration.switches) {
      platform.logger.info(
        `[${groupConfiguration.name}] Adding switch ${switchConfiguration.name}`,
      );

      // Creates the service and characteristic
      let switchService: Service;
      if (groupConfiguration.displayAsPowerStrip) {
        switchService = accessory.useService(
          Homebridge.Services.Outlet,
          switchConfiguration.name,
          `${switchConfiguration.name}-outlet`,
        );
      } else {
        switchService = accessory.useService(
          Homebridge.Services.Switch,
          switchConfiguration.name,
          `${switchConfiguration.name}-switch`,
        );
      }
      const onCharacteristic = switchService.useCharacteristic<boolean>(
        Homebridge.Characteristics.On,
      );
      ...
      onCharacteristic.valueChanged = (newValue) => {
        if (onCharacteristic.value !== newValue) {
          platform.logger.info(
            `[${groupConfiguration.name}] switch ${switchConfiguration.name} changed to ${newValue}`,
          );

          // Checks if there are already switches which are on
          const otherOnCharacteristics = this.onCharacteristics.filter(
            (c) =>
              c.configuration.name !== switchConfiguration.name &&
              c.characteristic.value,
          );
          for (const otherOnCharacteristic of otherOnCharacteristics) {
          ...
          if (
            newValue &&
            (groupConfiguration.timeout || switchConfiguration.timeout)
          ) {
          ...
          if (!newValue && switchConfiguration.nextOnSwitchName) {
            const nextOnCharacteristic = this.onCharacteristics.find(
              (c) =>
                c.configuration.name === switchConfiguration.nextOnSwitchName,
            );
            if (nextOnCharacteristic) {
              // Turns on the default characteristic
              this.platform.logger.info(
                `[${this.groupConfiguration.name}] next on activated: ${nextOnCharacteristic.configuration.name}`,
              );
              setTimeout(
   ...
   */
  private onCharacteristics = new Array<{
    configuration: SwitchConfiguration;
  ...
  private ensureDefaultOn() {
    ...
    const defaultOnCharacteristic = this.onCharacteristics.find(
      (c) => c.configuration.isDefaultOn,
    );
    ...
    // Turns on the default characteristic
    this.platform.logger.info(
      `[${this.groupConfiguration.name}] default on activated`,
    );
  ...
  private updateTimeout(switchConfiguration: SwitchConfiguration) {
    ...
    this.timeoutHandle = setTimeout(() => {
      for (const onCharacteristic of this.onCharacteristics) {
      ...
      if (switchConfiguration.nextOnSwitchName) {
        const nextOnCharacteristic = this.onCharacteristics.find(
          (c) => c.configuration.name === switchConfiguration.nextOnSwitchName,
        );
        if (nextOnCharacteristic) {
          // Turns on the default characteristic
          this.platform.logger.info(
          ...
          setTimeout(() => {
            if (
              this.groupConfiguration.timeout ||
              nextOnCharacteristic.configuration.timeout
            ) {
